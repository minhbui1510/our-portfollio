...
export default function ProfileCard({data}: { data?: ProfileData }) {
    ...
    return (
        <div
            className="max-w-3xl mx-auto p-6 bg-[rgb(var(--primary)/0.05)] rounded-2xl flex flex-col sm:flex-row items-center gap-6 shadow-md">
            <img
                src={data?.avatarUrl || ""}
                alt={data?.name || ""}
            ...
            <div className="flex-1 text-[rgb(var(--primary))]">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-[rgb(var(--text))]">
                    ...
                    <a href={`tel:${data?.phone}`} className="hover:text-[rgb(var(--accent))]">
                    <div className="flex items-center gap-2">
                        <FaPhoneAlt className="text-[rgb(var(--primary))]"/> {data?.phone}
                    </div>
                    </a>
                    <a href={`mailto:${data?.email}`} className="hover:text-[rgb(var(--accent))]">
                         <div className="flex items-center gap-2">
